...
This document explains the low-cost active-passive architecture implemented in this repository.
...
Internet
  -> External DNS provider
     -> Primary Elastic IP   -> EC2 primary   -> Caddy -> Nest.js image
     -> Secondary Elastic IP -> EC2 secondary -> Caddy -> Nest.js image
```
...
- the primary node serves all traffic
- the secondary node is warm and ready for failover
...
- true active-active request distribution
- health-based traffic steering from AWS itself
- instant or transparent failover
- fully seamless TLS behavior under all failover scenarios

## External DNS Setup

Because your DNS is hosted outside AWS, you have two practical modes.
...
### Mode 2: Provider-Managed Failover
...
3. generates an Ansible inventory with `primary` and `secondary`
